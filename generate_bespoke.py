...
def write_citations(tag, date_str, all_articles, script, debate_summary, output_dir):
    ...
    citations = {
        ...
        "credits": {
            ...
            "text_to_speech": "Azure Neural TTS (Ava · Andrew)" if USE_AZURE_TTS else "OpenAI TTS API (Nova · Echo)",
...
def write_show_notes(tag, date_str, all_articles, debate_summary, output_dir):
    ...
    lines += [
        ...
        f"*Generated by Claude (Anthropic) · Script Review: {POLISH_MODEL} · Audio by {'Azure Neural TTS' if USE_AZURE_TTS else 'OpenAI TTS'} · Sources via Brave Search*",
    ]
...
def generate_bespoke_rss_feed(base_url):
    ...
    for audio_file in audio_files:
        ...
        tts_label = "Azure Neural TTS" if USE_AZURE_TTS else "OpenAI TTS"
        description += (
            f"<p><em>Generated by Claude (Anthropic) · Audio by {tts_label} · "
