...
def generate_meta_moment_text(changelog: str) -> str:
    ...
    prompt = (
        ...
        "That depth is roughly two turns per change: one naming what changed and the tech "
        "behind it, one flagging the caveat that comes with it — a fallback it can drop to, "
        "a tool actually doing the work, a limit worth knowing. A change with no real "
        "caveat gets one turn and a reaction, not a stretched second beat to fill the "
        "pattern. Don't let either turn run past a sentence or two — this is a beat, not an "
        "explainer.\n\n"
