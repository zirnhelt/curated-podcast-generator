...
1. INTRODUCTIONS — SAY WHAT THEY ARE, PLAINLY, NOT WHO THEY'RE PRETENDING TO BE:
   ...
   - REQUIRED somewhere in the script (here, or in the sign-off if it flows more naturally there — but it must land once, in full, not get crowded out): they tell new listeners exactly how to reach the show — both the email, {podcast_config['email']}, AND the website, {podcast_config['url']} — framed as a standing, genuine invitation to send corrections, feedback, or just say hello
    ...
    try:
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            if use_intro:
                ...
                print(f"  Added intro music: {INTRO_MUSIC.name} ({len(theme)/1000:.1f}s, trimmed to 10s)")
